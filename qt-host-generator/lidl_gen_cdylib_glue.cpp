                                 ...
                                 int maxWorkers)
{
    ...
    // lidl() takes no arguments. Extra ones get the invalid_args rejection the
    // generated C++ and Rust dispatches give any zero-parameter method.
    const QString builtInLidl =
        "    if (methodName == QStringLiteral(\"lidl\")) {\n"
        "        if (!args.isEmpty()) {\n"
        "            nlohmann::json err{{\"code\", \"invalid_args\"},\n"
        "                               {\"message\", \"expected 0 arguments, got \" + std::to_string(args.size())},\n"
        "                               {\"origin\", " + cppStringLiteral(qs(module.name)) + "}};\n"
        "            return logos::nlohmannToQVariant(err);\n"
        "        }\n"
        "        return QString::fromUtf8(" + cppStringLiteral(lidlDocument) + ");\n"
        "    }\n";
    ...
    s << "        if (doc.isArray()) methods = doc.array();\n";
    s << "    }\n";
    s << "    for (const QJsonValue& value : methods)\n";
    s << "        if (value.toObject().value(QStringLiteral(\"name\")).toString() == QStringLiteral(\"lidl\"))\n";
    s << "            return methods;\n";
    s << "    QJsonObject builtin;\n";
    s << "    builtin[QStringLiteral(\"name\")] = QStringLiteral(\"lidl\");\n";
    s << "    builtin[QStringLiteral(\"signature\")] = QStringLiteral(\"lidl()\");\n";
    // getMethods() is the Qt-host introspection surface: LIDL tstr is
    // represented as QString here, exactly as current C++/Rust providers map
    // every other tstr return. The canonical .lidl still spells it `tstr`.
    s << "    builtin[QStringLiteral(\"returnType\")] = QStringLiteral(\"QString\");\n";
    s << "    builtin[QStringLiteral(\"isInvokable\")] = true;\n";
    s << "    builtin[QStringLiteral(\"description\")] = QStringLiteral(\"The module's canonical LIDL interface document.\");\n";
    s << "    builtin[QStringLiteral(\"parameters\")] = QJsonArray();\n";
    s << "    methods.append(builtin);\n";
    s << "    return methods;\n";
    s << "}\n\n";
