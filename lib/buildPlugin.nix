...

{
  ...
  let
    ...
    # Collect include directories from module dependencies. Each `dep`
    # is a struct exposed by mkLogosModule's resolvedModuleDeps:
    #   { default, lib, headers-qt, headers-std }
    # Older deps that don't carry the struct still work — the resolver
    # falls back to the `default` package for every field.
    moduleDepIncludes = lib.concatMapStringsSep "\n" (name:
      let
        dep = moduleDeps.${name} or null;
        depHeaders =
          if dep == null then null
          else if dep ? "headers-${apiStyle}" then dep."headers-${apiStyle}"
          else dep;
      in if depHeaders != null then ''
        if [ -d "${depHeaders}/include" ]; then
          echo "Copying ${apiStyle}-typed include files from ${name}..."
  ...
  in pkgs.stdenv.mkDerivation (commonArgs // {
    ...
    preConfigure = ''
      ...
      logos-cpp-generator --metadata metadata.json --general-only \
        --api-style ${apiStyle} \
        --output-dir ./generated_code ${interfaceArgs}
