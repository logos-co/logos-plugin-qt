...
in {
  ...
  in pkgs.stdenv.mkDerivation (commonArgs // {
    ...
    installPhase = ''
      ...
      # Native cdylibs have no Qt metadata section. Ship the same stamped
      # manifest beside every module image so the Qt-free registry can inspect
      # it without loading code. The native host still verifies name() from the
      # binary before granting tokens, so this sidecar is discovery data rather
      # than an identity assertion on its own.
      if [ -f "$LOGOS_MODULE_SOURCE_DIR/metadata.json" ]; then
        cp "$LOGOS_MODULE_SOURCE_DIR/metadata.json" \
           "$out/lib/${config.name}_plugin.metadata.json"
      fi

      # Optional: typed replica factory plugin (generated by logos_module REP_FILE).
      #
      # This stays here even though the LogosView*.in templates it is built
      # from do not: those are logos-view-module's, but the LIBRARY they
      # produce is a build artifact of the module tree this function compiles,
      # exactly like the _plugin library above. Packaging what a plugin build
      # emits is this backend's job, and it needs no view knowledge to do it —
      # only the output filename convention.
      for _rf in \
